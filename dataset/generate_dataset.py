...
"""
File containing a set of function needed to design the experimental dataset
"""

"""
This function transforms an input array image and returns the transformed image.
The transformations applied are : 
- offsets (x and y, value chosen randomly between 0 and the 1/4th of the image size
- rotation (value chosen randomly between 0 and 360 degrees)
- horizontal and vertical flip (performed randomly)
"""
...
parser.add_argument('--test_count', default = 8000, help = "Number of (positive) samples in the testing dataset", type = int)

parser.add_argument('--crop_background', default = False, help = "Indicate whether the background images should be cropped to fit ResNet requirements.", type = bool)

parser.add_argument('--background_directory', default = 'background_images', help = "Location of the background images")
parser.add_argument('--target_directory', default = 'panels_in_the_wild', help = "Location of the target images")
parser.add_argument('--arrays_directory', default = 'arrays', help = "Location of the arrays ")
...
def generate_dataset(overall_count, target_directory, background_directory, array_directory, crop = False):
    ...
    # cases
    array_types = ['LB', 'LN', 'SB', 'SN']
    background_types = ['forest', 'fields']
    ...
    # intermediate level
    # we have six dataframes to generate : fields/forest, large/small and blue/black
    ...
    field_frames_names = [label for label in label_files if label[3:-11] == 'fields']
    forest_frames_names = [label for label in label_files if label[3:-11] == 'forest']    
    
    dataframes = [black_frames, blue_frames, small_frames, large_frames, field_frames, forest_frames]
    cases = ['black', 'blue', 'small', 'large', 'fields', 'forest']
...
def generate_complete_dataset(counts, target_directory, background_directory, arrays_directory, crop = False):
    ...
    for count, dataset in zip(counts, ["train", "validation", "test"]) : 
