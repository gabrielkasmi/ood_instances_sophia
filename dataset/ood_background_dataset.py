...
"""
Generates a test set with 16k samples of the four types of instances, with the urban background, unseen
in the previous PITW dataset.
"""
...
def generate_dataset(overall_count, target_directory, background_directory, array_directory, crop = False):
    ...
    dataframes = [black_frames, blue_frames, small_frames, large_frames]
    cases = ['black', 'blue', 'small', 'large']
...
def generate_complete_dataset(counts, target_directory, background_directory, arrays_directory, crop = False):
    ...
    for count, dataset in zip(counts, ["test_ood_background"]) : 
