...
corresponding_array_labels = {
    'LB' : {'source_domain' :  ['LB_fields_labels.csv', 'LB_forest_labels.csv'],
             'ood' : 'LB_urban_labels.csv'
    },
    'LN' : {'source_domain' :  ['LN_fields_labels.csv', 'LN_forest_labels.csv'],
             'ood' : 'LN_urban_labels.csv'
    },
    'SB' : {'source_domain' :  ['SB_fields_labels.csv', 'SB_forest_labels.csv'],
             'ood' : 'SB_urban_labels.csv'
    },
    'SN' : {'source_domain' :  ['SN_fields_labels.csv', 'SN_forest_labels.csv'],
             'ood' : 'SN_urban_labels.csv'
    },
    'large' : {'source_domain' : ['field_large_labels.csv', 'forest_large_labels.csv'],
                'ood' : 'large_labels.csv'
    },
    'small' : {'source_domain' : ['field_small_labels.csv', 'forest_small_labels.csv'],
                'ood' : 'small_labels.csv'
    },
    'blue' : {'source_domain' : ['field_blue_labels.csv', 'forest_blue_labels.csv'],
                'ood' : 'blue_labels.csv'
    },
    'black' : {'source_domain' : ['field_black_labels.csv', 'forest_black_labels.csv'],
                'ood' : 'black_labels.csv'
    },
}

# Set up the in and out domain label files. 
array_labels = corresponding_array_labels[args.array_type]['source_domain']
...
out_domain_label = corresponding_array_labels[args.array_type]['ood']
...
ood_test_path = os.path.join(dataset_dir, "test_ood_background")
annotations_test_ood = os.path.join(ood_test_path, out_domain_label)
...
ood_test_data = dataset.BDPVClassificationDataset(annotations_test_ood, ood_test_path, transform = transforms, max_count = args.max_count)
...
dataloader = {"train": train, 'val' : val, 'test' : test, 'test_ood' : ood_test_data}
...
def ood_evaluation(best_model, dataloader):
    ...
    with torch.no_grad():
        ...
        for data in tqdm(dataloader["test_ood"]):
            # i +=1
            images, labels, _ = data
...
def main():
    ...
    for i in range(args.n_iter):
        ...
        ood_outputs = ood_evaluation(best_model, dataloader)
