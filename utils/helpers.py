...
def confusion(prediction, truth):
    ...
    except : # If the prediction and labels are not tensors but arrays

        true_positives = len(np.where(confusion_vector == 1)[0])
        false_positives = len(np.where(confusion_vector == float('inf'))[0])
        true_negatives = sum(np.isnan(confusion_vector))
        false_negatives = len(np.where(confusion_vector == 0)[0])      
...
def return_f1(precision, recall):
    ...
    return 2 * np.divide((np.array(precision) * np.array(recall)), (np.array(precision) + np.array(recall)))
